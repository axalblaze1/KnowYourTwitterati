...
def establish_connection():
    global auth
    global api
    auth = tweepy.OAuthHandler("Put Key/token", "Put Key/token")
    auth.set_access_token('Put Key/token', 'Put Key/token')
    api = tweepy.API(auth)
...
def files_include():
    ...
    global ann
    global sc
    face_cascade = cv2.CascadeClassifier('neural_asset/haarcascade_frontalface_default.xml')
    face_cascade_side = cv2.CascadeClassifier('neural_asset/haarcascade_profile.xml')
    body_cascade = cv2.CascadeClassifier('neural_asset/haarcascade_fullbody.xml')
    ann = tf.keras.models.load_model('neural_asset/final_model/final_model_loc.h5')
    sc = load('neural_asset/final_model/scaler_loc.joblib')
...
def complete_user_analysis(screen_name):
    ...
    files_include()
    output = ""
    followers = tweepy.Cursor(api.followers, screen_name).items(1)  
    ...
    for follower in followers:
        ...
        dp = dp +[follower.profile_image_url]
        un = un + [follower.name]
    ...
    dp_url = user_acc.profile_image_url
    week0 = user[8]
    week1 = user[9]
    week2 = user[10]
    week3 = user[11]
    ...
    per_index = x1[0][0]
    col_index = Average(res)
    retweet_perc = user[8]
    retweet_perc = retweet_perc/(retweet_perc +1)
    ...
    output = output + user_name + "_flag_sep_esc_"
    output = output + dp_url + "_flag_sep_esc_"
    output = output + desc + "_flag_sep_esc_"
    output = output + dp_url + "_flag_sep_esc_"
    output = output + str(follwer_count) + "_flag_sep_esc_"
    output = output + str(following_count) + "_flag_sep_esc_"
    output = output + str(created_at) + "_flag_sep_esc_"
    output = output + str(per_index) + "_flag_sep_esc_"
    output = output + str(col_index) + "_flag_sep_esc_"
    output = output + str(retweet_perc) + "_flag_sep_esc_"
    output = output + str(noTweet) + "_flag_sep_esc_"
    output = output + str(following_count) + "_flag_sep_esc_"
    output = output + str(keyPTweet) + "_flag_sep_esc_"
    output = output + str(clusters) + "_flag_sep_esc_"
    output = output + str(TPcluster) + "_flag_sep_esc_"
    output = output + str(noTweetPDay) + "_flag_sep_esc_"
    output = output + str(following_count) + "_flag_sep_esc_"
    output = output + str(g0) + "##" + str(g1) + "##" + str(g2) + "##" + str(g3) + "##" + str(g4) + "##" + str(g5) + "##" + str(g6) + "##" + str(g7) + "##" + str(g8) + "##" + str(g9) + "##" + str(g10) + "_flag_sep_esc_"
    output = output + str(genu) + "##" + str(collusive) + "##" + str(unsure) + "_flag_sep_esc_"
    output = output + str(week0) + "##" + str(week1) + "##" + str(week2) + "##" + str(week3) + "##" + str(week4) + "##" + str(week5) + "##" + str(week5) + "_flag_sep_esc_"
    output = output + listjoin(un) + "_flag_sep_esc_"
    output = output + listjoin(res) + "_flag_sep_esc_"
    output = output + listjoin(sn) + "_flag_sep_esc_"
    output = output + listjoin(dp) + "_flag_sep_esc_"
    # print(output)
    # print(5)
